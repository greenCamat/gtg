 <header class="page-header clearfix">
    <nav class="header-navigation">
    	<h1 class="logo"><a href="index.html">GTG</a></h1>
        <!--<ul>
            <li><a href="#" class="btn-new-contact" title="Edit Profile"></a></li>
            <li><a href="#" class="btn-settings" title="Settings"></a></li>
    	</ul>-->
    </nav>
	
	<div class="header-menu">
		<nav id="navigation">
			<ul>
				<li><a href="{{ url ('/') }}" title="Home">Home</a></li>
				<li><a href="{{ url ('shop') }}" title="Shop">Shop</a></li>
				<li><a href="#" title="Sale">Sale</a></li>
				<li><a href="#" title="Packages">Our Packages</a></li>
				<li><a href="{{ url ('aboutus') }}" title="About Us">About Us</a></li>
				<li><a href="{{ url ('contactus') }}" title="Contact Us">Contact Us</a></li>
				<!--
				<li><a href="#" title="My Wallet">My Wallet</a></li>
				<li><a href="login.html" title="Logout">Logout</a></li>-->
			</ul>
		</nav>
	</div>
	<div class="header-right">
		<div class="socials">
			<a href="#"><img src="{{ url ('images/icon/media/facebook.png') }}" title="facebook" alt="facebook"/>facebook</a>
			<a href="#"><img src="{{ url ('images/icon/media/instagram.png') }}" title="instagram" alt="instagram"/>instagram</a>
			<a href="#"><img src="{{ url ('images/icon/media/twitter.png') }}" title="twitter" alt="twitter"/>twitter</a>
		</div>
	</div>
</header>